...
def load_pretrained(model, checkpoint_path):
    """
    Load pretrained weights from an MMAction2 ST-GCN checkpoint into our model.

    MMAction2 checkpoints use different key naming conventions:
        backbone.st_gcn_networks.0.gcn.conv.weight
    Our model uses:
        blocks.0.gcn.conv.weight

    This function remaps the keys automatically before loading.
    ...
    # ── Remap MMAction2 key names to our key names ──────────────────
    # backbone.st_gcn_networks.N.*  →  blocks.N.*
    # backbone.data_bn.*            →  data_bn.*
    # backbone.A                    →  A  (adjacency — skipped, shape mismatch)
    # cls_head.*                    →  head.*  (skipped, different shape)
    remapped = {}
    for k, v in state_dict.items():
        if k.startswith("backbone.st_gcn_networks."):
            # backbone.st_gcn_networks.0.gcn.conv.weight → blocks.0.gcn.conv.weight
            new_k = k.replace("backbone.st_gcn_networks.", "blocks.")
            remapped[new_k] = v
        elif k.startswith("backbone.data_bn."):
            # backbone.data_bn.weight → data_bn.weight
            new_k = k.replace("backbone.", "")
            remapped[new_k] = v
        # Skip backbone.A (shape mismatch — different joint count)
        # Skip cls_head.* (classification head — replaced by our regression head)

    model_dict = model.state_dict()

    # Only load keys that exist in our model AND have matching shapes
    compatible = {}
    shape_mismatch = []
    not_in_model = []
    ...
    loaded_pct = len(compatible) / total * 100
    print(f"Pretrained checkpoint: {checkpoint_path}")
    print(f"Loaded:  {len(compatible)}/{total} layers ({loaded_pct:.0f}%)")

    if shape_mismatch:
        print(f"Skipped {len(shape_mismatch)} shape mismatches (expected — different joint count):")
        for k, pretrained_shape, our_shape in shape_mismatch[:5]:
            print(f"  {k}: checkpoint={pretrained_shape} vs ours={our_shape}")

    if not_in_model:
        print(f"Skipped {len(not_in_model)} keys not in our model (expected — head layers)")

    if loaded_pct < 50:
        print("WARNING: fewer than 50% of layers loaded. Check checkpoint format.")
    else:
        print("Backbone successfully initialized from pretrained weights.")
